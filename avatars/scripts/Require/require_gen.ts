...
const colors: {[char: string]: string} = {
...
function colorize(str: string): string {
  return str.replace(/§(.)/g, (_, char: string) => `\x1b[${colors[char]}m`) + "\x1b[0m";
}
  ...
  case 3:
    switch (process.argv[2]) {
      case "error":
        console.log(
          colorize(
            "§cThis script is not running in the 'figura/model_files' folder§r:\n"
      ...
      case "where":
        console.log(
          colorize(
            `The files are stored at §e${path.resolve(CWD, "../stored_vars")}§r.`
          )
        );
        break;
      default:
...
const LUAPATH = path.resolve(CWD, process.argv[2]),
      MODPATH = path.resolve(CWD, "../stored_vars/LUAMODULE_" + process.argv[3]);
...
let LUADATA: string;
try {
  LUADATA = readFileSync(LUAPATH).toString();
} catch {
...
try {
  writeFileSync(MODPATH + ".json", JSONDATA);
  writeFileSync(MODPATH + ".lua", `return [[${process.argv[3]}]]`);
} catch {
