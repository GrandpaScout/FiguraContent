...
{
    const BLANK_EVENT = {};
    const MAIN_CONTROLS = main_preview.controls;
    ...
    const REMOVEPOS_ACTION = {
        ...
        condition: (self) => (NullObject.selected.length === 1) && Modes.animate && self === savedData.nodepos.self,
        click: (self) => {
            const NODEPOS = savedData.nodepos;
    ...
    const REMOVETGT_ACTION = {
        ...
        condition: (self) => (NullObject.selected.length === 1) && Modes.animate && self === savedData.nodetgt.self,
        click: (self) => {
            const NODETGT = savedData.nodetgt;
    ...
    const FOLLOW_MENU = {
        ...
        condition: () => (NullObject.selected.length === 1) && Modes.animate,
        click: (self) => {
            const NODEPOS = savedData.nodepos.self, NODETGT = savedData.nodetgt.self;
            if (self === NODEPOS)
  ...
  `);
    ...
    const FOLLOW_TOGGLE = new ConditionalToggle("gs-camfollow_toggle", {
        ...
        onChange: (state) => {
            if (state) {
                let tgt = savedData.camera.target, pos = savedData.camera.position;
                ...
                let tgt = MAIN_CONTROLS.target, pos = MAIN_CAMERA.position, stgt = savedData.camera.target, spos = savedData.camera.position;
                stgt[0] = tgt.x,
                    stgt[1] = tgt.y,
                    stgt[2] = tgt.z,
                    spos[0] = pos.x,
                ...
                let ntgt = savedData.nodetgt.lastPos, npos = savedData.nodepos.lastPos;
                ntgt[0] = NaN,
                    ntgt[1] = NaN,
                    ntgt[2] = NaN,
                    npos[0] = NaN,
    ...
    function shouldFollowFail(newState) {
        if (newState ?? FOLLOW_TOGGLE.value)
            return "Camera Follow disabled.";
        if (!Modes.animate)
            return "Not in Animate Mode.";
        let npos = savedData.nodepos, ntgt = savedData.nodetgt;
    ...
    function cameraFollow(force) {
        if (force ?? FOLLOW_TOGGLE.value)
            return;
        ...
        let ltgt = savedData.nodetgt.lastPos, lpos = savedData.nodepos.lastPos, 
    ...
    BBPlugin.register("camerafollow", {
        ...
        icon: "fas.fa-video",
        onload() {
            const NULLOBJECT_MENU = NullObject.prototype.menu;
        ...
        onunload() {
            NullObject.prototype.menu.removeAction("gs-camfollow");
